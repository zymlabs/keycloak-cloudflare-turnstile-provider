...
/**
 * FormAction that injects Turnstile widget into registration form via JavaScript.
 * This is Option 2: Script Injection - works with any theme, no template modifications needed.
 *
 ...
 */
public class CloudflareTurnstileScriptInjectionAction implements FormAction {

    private static final Logger logger = Logger.getLogger(CloudflareTurnstileScriptInjectionAction.class);
    ...
    public void buildPage(FormContext context, LoginFormsProvider form) {
        ...
        if (config == null) {
            logger.warn("No authenticator configuration found for Script Injection");
            return;
        }
        ...
        try {
            ...
            logger.error("Failed to encode Turnstile injection script", e);
        }
    }
    ...
    private String buildInjectionScript(String siteKey, String widgetMode, String widgetTheme) {
        ...
        return String.format(
            ...
            "                 document.querySelector('form#kc-register-form') || " +
            "                 document.querySelector('form');" +
            "      if (!form) {" +
            "        console.warn('Turnstile: Could not find registration form');" +
            "        return;" +
            "      }" +
            ...
            "      console.log('Turnstile: Widget injected successfully');" +
            "      if (typeof turnstile !== 'undefined') {" +
            "        turnstile.render(widget);" +
            "      }" +
            "    } catch (e) {" +
            "      console.error('Turnstile: Error injecting widget:', e);" +
            "    }" +
            "  }" +
    ...
    public void success(FormContext context) {
        ...
        try {
            ...
            logger.debugf("Stored Turnstile verification result for registration (script injection) from IP %s", ipAddress);
        } catch (Exception e) {
            logger.warn("Failed to store Turnstile verification result for registration (script injection)", e);
        }
    }
